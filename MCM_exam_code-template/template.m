...
bTi = GetTransformationWrtBase(biTei, 6);
...
bOg = [0.2, 0, 0.15]';
etha = pi/6;
Ry = [cos(etha), 0 , -sin(etha); 0, 1, 0; sin(etha), 0, cos(etha)];
eRg = bTe(1:3, 1:3) * Ry;
bRg = eRg' * bTe(1:3, 1:3)';
bTg = [bRg, bOg; 0, 0, 0, 1];
...
for i = t
    ...
    bTi = GetTransformationWrtBase(biTei, numberOfLinks);
    bTe = bTi(:, :, numberOfLinks);    % computing end effector jacobian 
    bJe = GetJacobian(bTi, linkType);
    detbRe = det(bTe(1:3, 1:3));
	...
	if (rem(i,0.1) == 0) % only every 0.1 sec
		for n =1:numberOfLinks
			bTi = GetTransformationWrtBase(biTei,n);
		end
